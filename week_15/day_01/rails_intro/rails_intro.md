...
> Or show project 1 being done in a couple of mins
```bash
#terminal

rails new record_shop -d postgresql
cd record_shop
rails generate scaffold Artist name:string
rails generate scaffold Album title:string quantity:integer price:float artist:references
rake db:create
rake db:migrate
rails server 
```
