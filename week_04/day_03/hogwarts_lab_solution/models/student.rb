require_relative('../db/sql_runner')

class Student

  attr_accessor :age, :first_name, :last_name, :house_id
  attr_reader :id

  def initialize (options)
    @id = options['id'].to_i if options['id']
    @first_name = options['first_name']
    @last_name = options['last_name']
    @house_id = options['house_id'].to_i
    @age = options['age'].to_i
  end

  def save
    sql = "INSERT INTO students (first_name, last_name, house_id, age) VALUES ('#{@first_name}', '#{@last_name}', #{@house_id}, #{@age}) RETURNING *"
    result = SqlRunner.run(sql)
    id = result.first['id']
    @id = id
  end

  def house
    house = House.find(@house_id)
    return house
  end

  def update
    sql = "UPDATE students SET first_name = '#{@first_name}', last_name = '#{@last_name}', age = #{@age}, house_id = '#{@house_id}' WHERE id = #{@id}"
    SqlRunner.run(sql)
  end

  def delete
    sql = "DELETE FROM students WHERE id = #{@id}"
    SqlRunner.run(sql)
  end

  def self.all
    sql = "SELECT * FROM students"
    students = map_students(sql)
    return students
  end 

  def self.map_students(sql)
    students = SqlRunner.run(sql)
    return students.map { |student| Student.new(student) }
  end

  def self.find(id)
    sql = "SELECT * FROM students WHERE id = #{id}"
    result = SqlRunner.run(sql).first
    student = Student.new(result)
    return student
  end

  def format_name
    return "#{@first_name.capitalize} #{@last_name.capitalize}"
  end



end